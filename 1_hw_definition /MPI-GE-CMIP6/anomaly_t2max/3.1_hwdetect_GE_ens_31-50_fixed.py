...
def run_hw_detect(ens):   
    ...
    file = 'nGE_Tmax_'+ run_type[run] +'_dm_ens_31-50_land_europe_ens_mean_anomaly.nc'
...
for ens in range(ens_0,ens_n):
   ...
   #save data
    ...
    file = 'hw_detect_%id'%c_days +'_%s'%percentile+ '_' + def_type + '_GE_'+ run_type[run] +'_ref_%i'%ref_min + '-%i'%ref_max + '_ensemble_%i_anomaly.nc'%ens_total    
