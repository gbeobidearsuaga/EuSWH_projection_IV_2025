...
def run_hw_exceed(ens,run_type):    
    ...
    file = 'nGE_Tmax_'+ run_type +'_dm_ens_0-30_land_europe_ens_mean_anomaly.nc'
    ...
    file = 'threshold_%s'%percentile+ '_' + def_type +'_GE_ref_%i-'%ref_min + '%i_'%ref_max + 'ensemble_%i_anomaly.nc'%ens
    ...
    file = 'hw_detect_%id'%c_days +'_%s'%percentile+ '_' + def_type + '_GE_'+ run_type +'_ref_%i'%ref_min + '-%i'%ref_max + '_ensemble_%i_anomaly.nc'%ens   
...
for ens in range(ens_start,ens_end):
    ...
    file = 'data_exceed_lonlat_weighted_%s' %percentile + 'pct_%s' %c_days + 'd_' + def_type + '_GE_'+run_type[run]+'_ref_%i'%ref_min + '-%i'%ref_max + '_ensemble_%i_anomaly.nc'%ens   
