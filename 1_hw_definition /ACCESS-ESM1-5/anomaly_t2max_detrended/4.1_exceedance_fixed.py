...
def run_hw_exceed(ens,run_type):    
    ...
    file = 'hw_detect_%id'%c_days +'_%s'%percentile+ '_' + def_type + '_'+model+'_'+ run_type +'_ref_%i'%ref_min + '-%i'%ref_max + '_ensemble_%i_anomaly_detrend.nc'%ens  
...
for run in np.arange(len(run_type)):
    for ens in np.arange(ens_number):
        ...
        #save data   
        path = '/work/uo1075/u241308/data_python/HW_extension/'+model+'/anomaly_detrended/hw_exceed/'
        file = 'data_exceed_lonlat_weighted_%s' %percentile + 'pct_%s' %c_days + 'd_' + def_type + '_'+model+'_'+run_type[run]+'_ref_%i'%ref_min + '-%i'%ref_max + '_ensemble_%i_anomaly_detrend.nc'%ens   
